...
short_description: Update yum configuration files for deployments.
...
def run_module():
    try:
        import ansible_collections.repo_setup.repos.plugins.module_utils.repo_setup.yum_config.constants as const
        from ansible_collections.repo_setup.repos.plugins.module_utils.repo_setup.yum_config import (
            utils,
        )
    except ImportError:
        import repo_setup.yum_config.constants as const
        from repo_setup.yum_config import utils

    supported_config_types = ["repo", "global", "module", "enable-compose-repos"]
    ...
    module_args = dict(
        ...
        centos_release=dict(type="str", choices=const.COMPOSE_REPOS_RELEASES),
        arch=dict(
            type="str", choices=const.COMPOSE_REPOS_SUPPORTED_ARCHS, default="x86_64"
        ),
        variants=dict(type="list", default=[], elements="str"),
        disable_conflicting_variants=dict(type="bool", default=False),
        disable_repos=dict(type="list", default=[], elements="str"),
    )
    required_if_params = [
        ["type", "module", ["name"]],
    ...
    if six.PY2 and module.params["type"] in operations_not_supp_in_py2:
        msg = (
            "The configuration type '{0}' is not " "supported with python 2."
        ).format(module.params["type"])
        module.fail_json(msg=msg)

    if (
        module.params["type"] == "repo"
        and not module.params["name"]
        and not module.params["down_url"]
    ):
        msg = (
            "When using configuration type '{0}' you must provide a repo "
            "'name' or a 'down_url'."
        ).format(module.params["type"])
        module.fail_json(msg=msg)
    ...
    try:
        try:
            import ansible_collections.repo_setup.repos.plugins.module_utils.repo_setup.yum_config.yum_config as cfg
        except ImportError:
            import repo_setup.yum_config.yum_config as cfg

        if module.params["type"] == "repo":
            config_obj = cfg.YumRepoConfig(
            ...
            config_obj = cfg.YumGlobalConfig(
                file_path=module.params["file_path"],
                environment_file=module.params["environment_file"],
            )
            ...
            try:
                import ansible_collections.repo_setup.repos.plugins.module_utils.repo_setup.yum_config.compose_repos as repos
            except ImportError:
                import repo_setup.yum_config.compose_repos as repos

            # 1. Create compose repo config object
            repo_obj = repos.YumComposeRepoConfig(
            ...
            try:
                import ansible_collections.repo_setup.repos.plugins.module_utils.repo_setup.yum_config.dnf_manager as dnf_mgr
            except ImportError:
                import repo_setup.yum_config.dnf_manager as dnf_mgr

            dnf_mod_mgr = dnf_mgr.DnfModuleManager()
            if module.params["enabled"]:
                dnf_mod_mgr.enable_module(
                    module.params["name"],
                    stream=module.params["stream"],
                    profile=module.params["profile"],
                )
            else:
                dnf_mod_mgr.disable_module(
                    module.params["name"],
                    stream=module.params["stream"],
                    profile=module.params["profile"],
                )
            if module.params["operation"]:
                dnf_method = getattr(
                    dnf_mod_mgr, module.params["operation"] + "_module"
                )
                dnf_method(
                    module.params["name"],
                    stream=module.params["stream"],
