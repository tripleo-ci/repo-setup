...
def _parse_args(distro_id, distro_major_version_id):
    ...
    parser.add_argument('repos', metavar='REPO', nargs='+',
                        choices=['current', 'deps', 'current-tripleo',
                                 'current-repo-setup-dev', 'ceph', 'opstools',
                                 'repo-setup-ci-testing', 'current-repo-setup-rdo'],
...
def _validate_distro_repos(args):
    ...
    if 'fedora' in args.distro:
        ...
        valid_repos = ['ceph', 'current', 'current-tripleo',
                       'current-repo-setup-dev', 'deps', 'repo-setup-ci-testing',
                       'opstools', 'current-repo-setup-rdo']
...
def _validate_current_repo_setup.repos):
    ...
    if 'current' in repos or 'current-tripleo' in repos or 'deps' in repos:
        raise InvalidArguments('current-repo-setup-dev should not be used with '
...
def _validate_repo_setup_ci_testing(repos):
    ...
    if 'repo-setup-ci-testing' in repos and len(repos) > 1:
        if 'deps' in repos or 'ceph' in repos or 'opstools' in repos:
            ...
            raise InvalidArguments('Cannot use repo-setup-ci-testing at the '
...
def _remove_existing(args):
    ...
    if args.distro in ['ubi8', 'ubi9']:
        regex = '^(BaseOS|AppStream|delorean|repo-setup-centos-' \
                '(opstools|ceph|highavailability|powertools)).*.repo'
    else:
        regex = '^(delorean|repo-setup-centos-' \
                '(opstools|ceph|highavailability|powertools)).*.repo'
...
def _install_repos(args, base_path):
    ...
    for repo in args.repos:
        if repo == 'current':
            ...
            content = _get_repo(base_path + 'repo-setup-ci-testing/delorean.repo',
                                args)
            _write_repo(content, args.output_path)
            install_deps(args, base_path)
